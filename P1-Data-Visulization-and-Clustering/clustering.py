...
# 1.1 2D data vectors, 10000 multivariate normal
...
# 2.2 Hierarchicall clustering algorithm
...
# 2.3 Gaussian Mixture Model algorithm
# Same process as E2-T1, this time for
...
# 2.5 Centers of clusters closer, and best cluster algorithm?
...
# K-Means
kmeanscluster = KMeans(n_clusters=2).fit(xarr)
datapredictions = pd.concat([datapredictions, pd.DataFrame(kmeanscluster.predict(xarr2), columns=['km'])], axis=1)
# Hierarchicall
hierarcicallcluster = AgglomerativeClustering(n_clusters=2).fit(xarr2)
datapredictions = pd.concat([datapredictions, pd.DataFrame(hierarcicallcluster.fit_predict(xarr2), columns=['hi'])], axis=1)
# GMM
GMM = GaussianMixture(n_components=2, covariance_type='full').fit(xarr2)
datapredictions = pd.concat([datapredictions, pd.DataFrame(GMM.predict(xarr2), columns=['GM'])], axis=1)
...
sns.scatterplot(x='mu4 x', y='mu4 y', data=data2, label='mu4')
plt.subplot(2,3,4)
sns.scatterplot(x='x', y='y', data=datapredictions, hue='km')
plt.subplot(2,3,5)
sns.scatterplot(x='x', y='y', data=datapredictions, hue='hi')
plt.subplot(2,3,6)
sns.scatterplot(x='x', y='y', data=datapredictions, hue='GM')


# 2.6 Classes from 2 to 5, performance of methods?
# WORK TO BE DONE
...
# 3.2
# Apply GMM to the data in T1, and with BIC or AIC determine optimal number of clusters
